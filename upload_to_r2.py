...
憑證/設定一律從環境變數讀取（不寫死、不入庫）：
  R2_ACCOUNT_ID         Cloudflare 帳號 ID（組成 endpoint，也是 Access Key ID 來源）
  R2_BUCKET             目標 bucket 名稱
  Access Key ID：       R2_ACCESS_KEY_ID（= Cloudflare API token 的 ID）
  Secret Access Key（二擇一）：
    R2_SECRET_ACCESS_KEY  直接給 S3 Secret（建 R2 API token 時 Cloudflare 顯示的那串）
    R2_TOKEN              給 Cloudflare API token 值（cfat_...），程式自動算
                         secret = SHA-256(token)（Cloudflare R2 的規則）
選填：
...
def main():
    ...
    import hashlib
    need = ['R2_ACCOUNT_ID', 'R2_ACCESS_KEY_ID', 'R2_BUCKET']
    missing = [k for k in need if not os.environ.get(k)]
    # Secret 二擇一：直接給 R2_SECRET_ACCESS_KEY，或給 R2_TOKEN 由程式算 sha256
    secret = os.environ.get('R2_SECRET_ACCESS_KEY')
    if not secret and os.environ.get('R2_TOKEN'):
        secret = hashlib.sha256(os.environ['R2_TOKEN'].encode()).hexdigest()
    if not secret:
        missing.append('R2_SECRET_ACCESS_KEY 或 R2_TOKEN')
    if missing:
