...
class WORKER(object):
    def __init__(self, cfgs, run_name, Gen, Gen_mapping, Gen_synthesis, Dis, Gen_ema, Gen_ema_mapping, Gen_ema_synthesis,
        ...
        # Initialize image pools for CycleGAN
        if self.MODEL.backbone == "cyclegan":
            from utils.image_pool import ImagePool
            pool_size = self.cfgs.CYCLEGAN.pool_size
    ...
    def sample_unpaired_data_basket(self):
        """Sample unpaired data for CycleGAN training

        Returns 4 elements: (img_A_basket, label_A_basket, img_B_basket, label_B_basket)
        """
        try:
            img_A_basket, label_A_basket, img_B_basket, label_B_basket = next(self.train_iter)
        except StopIteration:
            self.epoch_counter += 1
            if self.RUN.train and self.DDP:
                self.train_dataloader.sampler.set_epoch(self.epoch_counter)
        ...
        img_A_basket = torch.split(img_A_basket, self.OPTIMIZATION.batch_size)
        label_A_basket = torch.split(label_A_basket, self.OPTIMIZATION.batch_size)
        img_B_basket = torch.split(img_B_basket, self.OPTIMIZATION.batch_size)
        label_B_basket = torch.split(label_B_basket, self.OPTIMIZATION.batch_size)
        return img_A_basket, label_A_basket, img_B_basket, label_B_basket

    def sample_paired_data_basket(self):
        """Sample paired data for Pix2Pix training

        Returns 3 elements: (img_A_basket, img_B_basket, label_basket)
        """
        try:
            img_A_basket, img_B_basket, label_basket = next(self.train_iter)
        except StopIteration:
            self.epoch_counter += 1
            if self.RUN.train and self.DDP:
                self.train_dataloader.sampler.set_epoch(self.epoch_counter)
        ...
        img_A_basket = torch.split(img_A_basket, self.OPTIMIZATION.batch_size)
        img_B_basket = torch.split(img_B_basket, self.OPTIMIZATION.batch_size)
        label_basket = torch.split(label_basket, self.OPTIMIZATION.batch_size)
        return img_A_basket, img_B_basket, label_basket

    def sample_ct_mask_basket(self, batch_counter):
        """Sample CT-mask paired data for StyleGAN3 Dual training

        Returns:
            ct_images: (batch_size, 3, H, W) - CT images
            mask_images: (batch_size, 1, H, W) - Binary masks [0, 1]
            labels: (batch_size,) - Class labels
        """
        try:
            ct_basket, mask_basket, label_basket = next(self.train_iter)
        except StopIteration:
            self.epoch_counter += 1
            if self.RUN.train and self.DDP:
                self.train_dataloader.sampler.set_epoch(self.epoch_counter)
        ...
        # Split baskets if using accumulation steps
        ct_basket = torch.split(ct_basket, self.OPTIMIZATION.batch_size)
        mask_basket = torch.split(mask_basket, self.OPTIMIZATION.batch_size)
        label_basket = torch.split(label_basket, self.OPTIMIZATION.batch_size)

        # Get the batch for current counter
        ct_images = ct_basket[batch_counter].to(self.local_rank, non_blocking=True)
        mask_images = mask_basket[batch_counter].to(self.local_rank, non_blocking=True)
        labels = label_basket[batch_counter].to(self.local_rank, non_blocking=True)

        return ct_images, mask_images, labels

    # -----------------------------------------------------------------------------
    # train CycleGAN Discriminator
    # -----------------------------------------------------------------------------
    def train_cyclegan_discriminator(self, current_step):
        """Train CycleGAN dual discriminators (Dis_A and Dis_B)

        Training flow:
        1. Get unpaired data: real_A and real_B
        2. Generate fake images: fake_B = Gen_A2B(real_A), fake_A = Gen_B2A(real_B)
        3. Use image pool to get historical fake images
        4. Calculate discriminator losses for both domains
        5. Update discriminators

        Returns:
            (0, dis_acml_loss): tuple matching standard GAN return format
        """
        batch_counter = 0
        # Make GAN trainable
        misc.make_GAN_trainable(self.Gen, self.Gen_ema, self.Dis)
        # Toggle gradients
        misc.toggle_grad(model=self.Gen, grad=False, num_freeze_layers=-1, is_stylegan=False)
        misc.toggle_grad(model=self.Dis, grad=True, num_freeze_layers=self.RUN.freezeD, is_stylegan=False)

        # Sample unpaired data (returns 4 elements)
        img_A_basket, label_A_basket, img_B_basket, label_B_basket = self.sample_unpaired_data_basket()

        # Get LSGAN loss function
        from utils.losses import LSGANLoss, patch_gan_discriminator_loss
        ...
        for step_index in range(self.OPTIMIZATION.d_updates_per_step):
            ...
            for acml_index in range(self.OPTIMIZATION.acml_steps):
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision else misc.dummy_context_mgr() as mpc:
                    # Load real images onto GPU
                    real_A = img_A_basket[batch_counter].to(self.local_rank, non_blocking=True)
                    real_B = img_B_basket[batch_counter].to(self.local_rank, non_blocking=True)
                    label_A = label_A_basket[batch_counter].to(self.local_rank, non_blocking=True)
                    label_B = label_B_basket[batch_counter].to(self.local_rank, non_blocking=True)

                    # Generate fake images
                    with torch.no_grad():
                        fake_B_dict = self.Gen(z=None, label=label_A, eval=False, domain='A', real_images=real_A)
                        fake_A_dict = self.Gen(z=None, label=label_B, eval=False, domain='B', real_images=real_B)
                        fake_B = fake_B_dict["fake_images"]
                        fake_A = fake_A_dict["fake_images"]

                    # Get images from history pool
                    fake_B_pooled = self.fake_B_pool.query(fake_B)
                    fake_A_pooled = self.fake_A_pool.query(fake_A)

                    # --- Update Discriminator A (discriminates domain A) ---
                    # Dis_A should classify real_A as real, fake_A as fake
                    real_A_dict = self.Dis(real_A, label_A, eval=False, domain='A')
                    fake_A_dict = self.Dis(fake_A_pooled.detach(), label_A, eval=False, domain='A')

                    loss_D_A = patch_gan_discriminator_loss(
                        real_A_dict["adv_output"],
                        fake_A_dict["adv_output"],
                        gan_loss_fn
                    )

                    # --- Update Discriminator B (discriminates domain B) ---
                    real_B_dict = self.Dis(real_B, label_B, eval=False, domain='B')
                    fake_B_dict = self.Dis(fake_B_pooled.detach(), label_B, eval=False, domain='B')

                    loss_D_B = patch_gan_discriminator_loss(
                        real_B_dict["adv_output"],
                        fake_B_dict["adv_output"],
                        gan_loss_fn
                    )

                    # Total discriminator loss (average of two domains)
                    dis_acml_loss = (loss_D_A + loss_D_B) * 0.5
                    dis_loss_total += dis_acml_loss.item()
        ...
        # Return format: (real_cond_loss, dis_loss) - no cond_loss for CycleGAN
        return 0, dis_loss_total / (self.OPTIMIZATION.d_updates_per_step * self.OPTIMIZATION.acml_steps)

    # -----------------------------------------------------------------------------
    # train CycleGAN Generator
    # -----------------------------------------------------------------------------
    def train_cyclegan_generator(self, current_step):
        """Train CycleGAN dual generators (Gen_A2B and Gen_B2A)

        Training flow:
        1. Forward cycle: real_A → fake_B → rec_A
        2. Backward cycle: real_B → fake_A → rec_B
        3. Identity mapping: real_B → idt_B (from Gen_A2B), real_A → idt_A (from Gen_B2A)
        4. Calculate GAN loss, cycle consistency loss, identity loss
        5. Update generators
        ...
        misc.make_GAN_trainable(self.Gen, self.Gen_ema, self.Dis)
        # Toggle gradients
        misc.toggle_grad(model=self.Dis, grad=False, num_freeze_layers=-1, is_stylegan=False)
        misc.toggle_grad(model=self.Gen, grad=True, num_freeze_layers=-1, is_stylegan=False)
        self.Gen.apply(misc.track_bn_statistics)

        # Get loss functions
        from utils.losses import (LSGANLoss, patch_gan_generator_loss,
                                   cycle_consistency_loss, identity_loss)
        gan_loss_fn = LSGANLoss()

        # Get CycleGAN hyperparameters
        lambda_A = self.cfgs.CYCLEGAN.lambda_A
        lambda_B = self.cfgs.CYCLEGAN.lambda_B
        lambda_identity = self.cfgs.CYCLEGAN.lambda_identity
        use_identity_loss = self.cfgs.CYCLEGAN.use_identity_loss
        ...
        for step_index in range(self.OPTIMIZATION.g_updates_per_step):
            ...
            for acml_step in range(self.OPTIMIZATION.acml_steps):
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision else misc.dummy_context_mgr() as mpc:
                    ...
                    fake_B_dict = self.Gen(z=None, label=label_A, eval=False, domain='A', real_images=real_A)
                    fake_B = fake_B_dict["fake_images"]
                    rec_A_dict = self.Gen(z=None, label=label_B, eval=False, domain='B', real_images=fake_B)
                    rec_A = rec_A_dict["fake_images"]

                    # --- Backward cycle: B → A → B ---
                    fake_A_dict = self.Gen(z=None, label=label_B, eval=False, domain='B', real_images=real_B)
                    fake_A = fake_A_dict["fake_images"]
                    rec_B_dict = self.Gen(z=None, label=label_A, eval=False, domain='A', real_images=fake_A)
                    rec_B = rec_B_dict["fake_images"]

                    # --- GAN loss ---
                    # Gen_A2B tries to fool Dis_B
                    fake_B_pred_dict = self.Dis(fake_B, label_B, eval=False, domain='B')
                    loss_GAN_A2B = patch_gan_generator_loss(fake_B_pred_dict["adv_output"], gan_loss_fn)

                    # Gen_B2A tries to fool Dis_A
                    fake_A_pred_dict = self.Dis(fake_A, label_A, eval=False, domain='A')
                    loss_GAN_B2A = patch_gan_generator_loss(fake_A_pred_dict["adv_output"], gan_loss_fn)

                    # --- Cycle consistency loss ---
                    loss_cycle = cycle_consistency_loss(rec_A, real_A, rec_B, real_B, lambda_A, lambda_B)

                    # --- Identity loss (optional) ---
                    loss_idt = 0
                    if use_identity_loss:
                        # Gen_A2B(real_B) should equal real_B (preserve B's identity)
                        idt_B_dict = self.Gen(z=None, label=label_B, eval=False, domain='A', real_images=real_B)
                        idt_B = idt_B_dict["fake_images"]

                        # Gen_B2A(real_A) should equal real_A (preserve A's identity)
                        idt_A_dict = self.Gen(z=None, label=label_A, eval=False, domain='B', real_images=real_A)
                        idt_A = idt_A_dict["fake_images"]

                        loss_idt = identity_loss(idt_A, real_B, idt_B, real_A,
                                                lambda_A, lambda_B, lambda_identity)

                    # --- Total generator loss ---
                    gen_acml_loss = loss_GAN_A2B + loss_GAN_B2A + loss_cycle + loss_idt
                    gen_loss_total += gen_acml_loss.item()
        ...
        return gen_loss_total / (self.OPTIMIZATION.g_updates_per_step * self.OPTIMIZATION.acml_steps)

    # -----------------------------------------------------------------------------
    # train Pix2Pix Discriminator
    # -----------------------------------------------------------------------------
    def train_pix2pix_discriminator(self, current_step):
        """Train Pix2Pix conditional discriminator

        Training flow:
        1. Get paired data: img_A (input) and img_B (target)
        2. Generate fake_B = Gen(img_A)
        3. Discriminator evaluates [img_A, real_B] vs [img_A, fake_B]
        ...
        # Sample paired data (returns 3 elements)
        img_A_basket, img_B_basket, label_basket = self.sample_paired_data_basket()

        # Get loss function (LSGAN or vanilla)
        from utils.losses import LSGANLoss, patch_gan_discriminator_loss
        # Check if using vanilla GAN or LSGAN
        if hasattr(self.cfgs, 'PIX2PIX') and hasattr(self.cfgs.PIX2PIX, 'gan_mode'):
            if self.cfgs.PIX2PIX.gan_mode == 'vanilla':
                gan_loss_fn = nn.BCEWithLogitsLoss()
            else:
                gan_loss_fn = LSGANLoss()
        else:
            gan_loss_fn = LSGANLoss()  # Default to LSGAN
        ...
        for step_index in range(self.OPTIMIZATION.d_updates_per_step):
            ...
            for acml_index in range(self.OPTIMIZATION.acml_steps):
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision else misc.dummy_context_mgr() as mpc:
                    # Load paired images onto GPU
                    img_A = img_A_basket[batch_counter].to(self.local_rank, non_blocking=True)
                    img_B = img_B_basket[batch_counter].to(self.local_rank, non_blocking=True)
                    label = label_basket[batch_counter].to(self.local_rank, non_blocking=True)

                    # Generate fake images
                    with torch.no_grad():
                        fake_B_dict = self.Gen(z=None, label=label, eval=False, real_images=img_A)
                        fake_B = fake_B_dict["fake_images"]

                    # Discriminate real pair [A, real_B]
                    real_pair_dict = self.Dis(img_B, label, eval=False, adc_fake=False, input_images=img_A)

                    # Discriminate fake pair [A, fake_B]
                    fake_pair_dict = self.Dis(fake_B.detach(), label, eval=False, adc_fake=False, input_images=img_A)

                    # Calculate discriminator loss
                    if isinstance(gan_loss_fn, nn.BCEWithLogitsLoss):
                        # Vanilla GAN: use BCE loss
                        loss_real = gan_loss_fn(real_pair_dict["adv_output"],
                                               torch.ones_like(real_pair_dict["adv_output"]))
                        loss_fake = gan_loss_fn(fake_pair_dict["adv_output"],
                                               torch.zeros_like(fake_pair_dict["adv_output"]))
                        dis_acml_loss = (loss_real + loss_fake) * 0.5
                    else:
                        # LSGAN
                        dis_acml_loss = patch_gan_discriminator_loss(
                            real_pair_dict["adv_output"],
                            fake_pair_dict["adv_output"],
                            gan_loss_fn
                        )
        ...
        # Return format: (real_cond_loss, dis_loss)
        return 0, dis_loss_total / (self.OPTIMIZATION.d_updates_per_step * self.OPTIMIZATION.acml_steps)

    # -----------------------------------------------------------------------------
    # train Pix2Pix Generator
    # -----------------------------------------------------------------------------
    def train_pix2pix_generator(self, current_step):
        """Train Pix2Pix generator with GAN loss and L1 reconstruction loss

        Training flow:
        1. Generate fake_B = Gen(img_A)
        2. Calculate GAN loss: fool discriminator
        3. Calculate L1 loss: |fake_B - real_B|
        4. Total loss = GAN + lambda_L1 * L1
        5. Update generator
        ...
        # Get loss functions
        from utils.losses import LSGANLoss, patch_gan_generator_loss, pix2pix_l1_loss
        ...
        # Get hyperparameters
        lambda_L1 = self.cfgs.PIX2PIX.lambda_L1
        ...
        for step_index in range(self.OPTIMIZATION.g_updates_per_step):
            ...
            for acml_step in range(self.OPTIMIZATION.acml_steps):
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision else misc.dummy_context_mgr() as mpc:
                    ...
                    # --- GAN loss ---
                    fake_pair_dict = self.Dis(fake_B, label, eval=False, adc_fake=False, input_images=img_A)

                    if isinstance(gan_loss_fn, nn.BCEWithLogitsLoss):
                        # Vanilla GAN
                        loss_GAN = gan_loss_fn(fake_pair_dict["adv_output"],
                                              torch.ones_like(fake_pair_dict["adv_output"]))
                    else:
                        # LSGAN
                        loss_GAN = patch_gan_generator_loss(fake_pair_dict["adv_output"], gan_loss_fn)

                    # --- L1 reconstruction loss ---
                    loss_L1 = pix2pix_l1_loss(fake_B, img_B, lambda_L1)

                    # --- Total generator loss ---
                    gen_acml_loss = loss_GAN + loss_L1
                    gen_loss_total += gen_acml_loss.item()
    ...
    # -----------------------------------------------------------------------------
    # StyleGAN3 Dual-Branch Training Methods
    # -----------------------------------------------------------------------------
    def train_stylegan3_dual_discriminator(self, current_step):
        """Train StyleGAN3 Dual discriminators for CT and mask images

        Training flow:
        1. Generate (fake_ct, fake_mask) = Gen(z)
        2. Compute CT discriminator loss: D_CT(real_ct) vs D_CT(fake_ct)
        3. Compute mask discriminator loss: D_mask(real_mask) vs D_mask(fake_mask)
        4. Combine losses based on fusion_strategy
        5. Update discriminators

        Returns:
            (0, dis_acml_loss): Tuple matching standard GAN format
        """
        # Make GAN trainable
        misc.make_GAN_trainable(self.Gen, self.Gen_ema, self.Dis)
        # Toggle gradients
        misc.toggle_grad(model=self.Gen, grad=False, num_freeze_layers=-1, is_stylegan=True)
        misc.toggle_grad(model=self.Dis, grad=True, num_freeze_layers=self.RUN.freezeD, is_stylegan=False)
        self.Dis.apply(misc.track_bn_statistics)

        # Get loss functions
        dis_loss = losses.d_logistic if self.LOSS.adv_loss == "logistic" else losses.d_hinge

        # Get hyperparameters
        fusion_strategy = self.cfgs.STYLEGAN3_DUAL.fusion_strategy
        lambda_ct = self.cfgs.STYLEGAN3_DUAL.lambda_ct
        lambda_mask = self.cfgs.STYLEGAN3_DUAL.lambda_mask
        ...
        for step_index in range(self.OPTIMIZATION.d_updates_per_step):
            ...
            for acml_step in range(self.OPTIMIZATION.acml_steps):
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision else misc.dummy_context_mgr() as mpc:
                    # Sample real CT-mask pairs
                    # IMPORTANT: StyleGAN3 Dual REQUIRES paired CT-mask data
                    # No fallback mechanism - will fail if translation_mode != "ct_mask"
                    if self.DATA.translation_mode != "ct_mask":
                        raise ValueError(
                            f"StyleGAN3 Dual requires translation_mode='ct_mask', but got '{self.DATA.translation_mode}'.\n"
                            f"Please set DATA.translation_mode: 'ct_mask' in your config file.\n"
                            f"Data structure required:\n"
                            f"  data_dir/\n"
                            f"  ├── ct/train/class0/*.png\n"
                            f"  └── masks/train/class0/*.png\n"
                            f"See CT_MASK_DATA_GUIDE.md for details."
                        )

                    real_ct, real_mask, real_labels = self.sample_ct_mask_basket(batch_counter)

                    # Generate fake images
                    z, fake_labels, trunc_psi = misc.sample_latents(
                        self.LOSS.latent_op,
                        self.MODEL.z_prior,
                        batch_size=real_ct.size(0),
                        z_dim=self.MODEL.z_dim,
                        num_classes=self.DATA.num_classes,
                        sigma_noise=self.LOSS.sigma_noise,
                        device=self.local_rank,
                        sampler=None
                    )

                    with torch.no_grad():
                        gen_output = self.Gen(z=z, label=fake_labels, eval=True, truncation_psi=trunc_psi)
                        fake_ct = gen_output["ct_image"]
                        fake_mask = gen_output["mask_image"]

                    # Discriminator forward pass
                    dis_output_real = self.Dis(ct_img=real_ct, mask_img=real_mask, label=real_labels)
                    dis_output_fake = self.Dis(ct_img=fake_ct, mask_img=fake_mask, label=fake_labels)

                    if fusion_strategy == "average":
                        # Simple average fusion
                        real_logits = dis_output_real["fused_logits"]
                        fake_logits = dis_output_fake["fused_logits"]
                        dis_acml_loss = dis_loss(real_logits, fake_logits)

                    elif fusion_strategy == "weighted":
                        # Weighted fusion
                        real_ct_logits = dis_output_real["ct_logits"]
                        real_mask_logits = dis_output_real["mask_logits"]
                        fake_ct_logits = dis_output_fake["ct_logits"]
                        fake_mask_logits = dis_output_fake["mask_logits"]

                        loss_ct = dis_loss(real_ct_logits, fake_ct_logits)
                        loss_mask = dis_loss(real_mask_logits, fake_mask_logits)

                        dis_acml_loss = lambda_ct * loss_ct + lambda_mask * loss_mask

                    elif fusion_strategy == "separate":
                        # Train CT and mask discriminators separately
                        real_ct_logits = dis_output_real["ct_logits"]
                        real_mask_logits = dis_output_real["mask_logits"]
                        fake_ct_logits = dis_output_fake["ct_logits"]
                        fake_mask_logits = dis_output_fake["mask_logits"]
                        ...
                        dis_acml_loss = loss_ct + loss_mask
    ...
    def train_stylegan3_dual_generator(self, current_step):
        """Train StyleGAN3 Dual generator with adversarial and consistency losses

        Training flow:
        1. Generate (fake_ct, fake_mask) = Gen(z)
        2. Compute GAN loss: fool discriminators
        3. Compute consistency loss: coherence between CT and mask
        4. Total loss = GAN + lambda_consistency * consistency
        5. Update generator
        ...
        # Toggle gradients
        misc.toggle_grad(model=self.Dis, grad=False, num_freeze_layers=-1, is_stylegan=False)
        misc.toggle_grad(model=self.Gen, grad=True, num_freeze_layers=-1, is_stylegan=True)
        self.Gen.apply(misc.track_bn_statistics)

        # Get loss functions
        gen_loss = losses.g_logistic if self.LOSS.adv_loss == "logistic" else losses.g_hinge
        from utils.losses import dual_branch_consistency_loss

        # Get hyperparameters
        fusion_strategy = self.cfgs.STYLEGAN3_DUAL.fusion_strategy
        lambda_ct = self.cfgs.STYLEGAN3_DUAL.lambda_ct
        lambda_mask = self.cfgs.STYLEGAN3_DUAL.lambda_mask
        lambda_consistency = self.cfgs.STYLEGAN3_DUAL.lambda_consistency
        ...
        for step_index in range(self.OPTIMIZATION.g_updates_per_step):
            ...
            for acml_step in range(self.OPTIMIZATION.acml_steps):
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision else misc.dummy_context_mgr() as mpc:
                    ...
                    # Sample latent codes (only need batch size from real data)
                    real_ct, real_mask, real_labels = self.sample_ct_mask_basket(batch_counter)
                    ...
                    # Generate fake images
                    gen_output = self.Gen(z=z, label=fake_labels, eval=False, truncation_psi=trunc_psi)
                    fake_ct = gen_output["ct_image"]
                    fake_mask = gen_output["mask_image"]
                    ...
                    if fusion_strategy == "average":
                        fake_logits = dis_output_fake["fused_logits"]
                        loss_GAN = gen_loss(fake_logits)

                    elif fusion_strategy == "weighted":
                        fake_ct_logits = dis_output_fake["ct_logits"]
                        fake_mask_logits = dis_output_fake["mask_logits"]

                        loss_GAN_ct = gen_loss(fake_ct_logits)
                        loss_GAN_mask = gen_loss(fake_mask_logits)

                        loss_GAN = lambda_ct * loss_GAN_ct + lambda_mask * loss_GAN_mask
                        ...
                        loss_GAN = loss_GAN_ct + loss_GAN_mask

                    else:
                        raise ValueError(f"Unknown fusion_strategy: {fusion_strategy}")

                    # --- Consistency loss ---
                    loss_consistency = dual_branch_consistency_loss(
                        ct_images=fake_ct,
                        mask_images=fake_mask,
                        lambda_consistency=lambda_consistency,
                        consistency_type="l1"  # Can be configured in YAML
                    )

                    # --- Total generator loss ---
                    gen_acml_loss = loss_GAN + loss_consistency
                    gen_loss_total += gen_acml_loss.item()
    ...
    def train_discriminator(self, current_step):
        # Route to specialized training methods for image translation models
        if self.MODEL.backbone == "cyclegan":
            return self.train_cyclegan_discriminator(current_step)
        elif self.MODEL.backbone == "pix2pix":
            return self.train_pix2pix_discriminator(current_step)
        elif self.MODEL.backbone == "stylegan3_dual":
            return self.train_stylegan3_dual_discriminator(current_step)

        # Standard GAN training logic below
        batch_counter = 0
    ...
    def train_generator(self, current_step):
        # Route to specialized training methods for image translation models
        if self.MODEL.backbone == "cyclegan":
            return self.train_cyclegan_generator(current_step)
        elif self.MODEL.backbone == "pix2pix":
            return self.train_pix2pix_generator(current_step)
        elif self.MODEL.backbone == "stylegan3_dual":
            return self.train_stylegan3_dual_generator(current_step)
    ...
    def visualize_fake_images(self, num_cols, current_step):
        ...
        if self.RUN.train:
            # New wandb expects [H,W,C] or [N,H,W,C]; we have [N,C,H,W] → make a grid instead.
            try:
                from torchvision.utils import make_grid
                grid = make_grid(fake_images.detach().cpu(), nrow=num_cols, normalize=True, value_range=(-1, 1))
                wandb.log({"generated_images": wandb.Image(grid)}, step=self.wandb_step)
            except Exception as e:
                self.logger.warning(f"wandb image log failed: {e}")
