...
def stylegan_generate_images(zs, fake_labels, num_classes, style_mixing_p, update_emas,
    ...
    # Handle dual-output synthesis (StyleGAN3 Dual)
    synthesis_output = generator_synthesis(ws, update_emas=update_emas)

    # StyleGAN3 Dual returns tuple (ct_img, mask_img), use CT image for evaluation
    if isinstance(synthesis_output, tuple):
        fake_images = synthesis_output[0]  # Use CT image only
    else:
        fake_images = synthesis_output  # Standard StyleGAN output
