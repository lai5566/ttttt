...
def _make_dim_pyramid(img_size: int, max_ch: int = 512, min_ch: int = 8):
    """Auto-derive in/out channel pyramids from img_size.

    Each block doubles spatial (4×4 -> ... -> img_size). Channels halve per step,
    clamped to [min_ch, max_ch]. Returns (in_dims, out_dims).

    img_size=32   -> 3 blocks ([512,256,128] / [256,128,64])  (matches original)
    img_size=64   -> 4 blocks
    img_size=128  -> 5 blocks
    img_size=256  -> 6 blocks
    img_size=512  -> 7 blocks
    """
    import math
    n_blocks = int(math.log2(img_size)) - 2
    assert 2 ** (n_blocks + 2) == img_size, f"img_size must be power of 2 ≥ 4; got {img_size}"
    in_dims, out_dims = [], []
    cur = max_ch
    for i in range(n_blocks):
        nxt = max(cur // 2, min_ch)
        in_dims.append(cur)
        out_dims.append(nxt)
        cur = nxt
...
class Generator(nn.Module):
    def __init__(self, z_dim, g_shared_dim, img_size, g_conv_dim, apply_attn, attn_g_loc, g_cond_mtd, num_classes, g_init, g_depth,
        ...
        self.in_dims, self.out_dims = _make_dim_pyramid(img_size)
...
class Discriminator(nn.Module):
    def __init__(self, img_size, d_conv_dim, apply_d_sn, apply_attn, attn_d_loc, d_cond_mtd, aux_cls_type, d_embed_dim, normalize_d_embed,
        ...
        # Mirror the generator pyramid for the discriminator (encoder).
        # Original 32x32: in=[3,64,128], out=[64,128,256]; final conv 256→512.
        g_in, g_out = _make_dim_pyramid(img_size)
        # Discriminator goes the other way: input channels = 3 (RGB), then growing.
        # Total feature channels at bottleneck = max(g_in) (= 512 by default).
        d_widths = list(reversed(g_out))                # e.g. [8,16,32,64,128,256] for 256x256
        # First disc block input is image channels (3), then takes from d_widths.
        self.in_dims = [3] + d_widths[:-1]
        self.out_dims = d_widths
        ...
        self.activation = MODULES.d_act_fn
        # Final conv lifts last block output to 512 features for the linear head.
