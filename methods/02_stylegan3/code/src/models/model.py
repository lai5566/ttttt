...
def load_generator_discriminator(DATA, OPTIMIZATION, MODEL, STYLEGAN, MODULES, RUN, device, logger):
    ...
    if MODEL.backbone in ["cyclegan", "pix2pix"]:
        ...
        # No EMA for translation models by default
        Gen_ema, Gen_ema_mapping, Gen_ema_synthesis, ema = None, None, None, None

    elif MODEL.backbone in ["stylegan2", "stylegan3"]:
    ...
    elif MODEL.backbone == "stylegan3_dual":
        # StyleGAN3 Dual-Branch for CT + Mask generation
        if device == 0:
            logger.info("Loading StyleGAN3 Dual-Branch architecture for simultaneous CT and mask generation.")

        # Import config object if available
        STYLEGAN3_DUAL = __import__("config", fromlist=["something"]).Configurations(None).STYLEGAN3_DUAL if hasattr(__import__("config", fromlist=["something"]).Configurations(None), "STYLEGAN3_DUAL") else None

        # Get configuration
        channel_base = 32768 if DATA.img_size >= 512 or DATA.name in ["CIFAR10", "CIFAR100"] else 16384
        channel_max = 512
        gen_c_dim = DATA.num_classes if MODEL.g_cond_mtd == "cAdaIN" else 0
        dis_c_dim = DATA.num_classes if MODEL.d_cond_mtd in STYLEGAN.cond_type else 0
        ...
        # StyleGAN3 configuration
        magnitude_ema_beta = 0.5 ** (OPTIMIZATION.batch_size * OPTIMIZATION.acml_steps / (20 * 1e3))
        g_channel_base, g_channel_max, conv_kernel, use_radial_filters = channel_base, channel_max, 3, False
        if STYLEGAN.stylegan3_cfg == "stylegan3-r":
            g_channel_base, g_channel_max, conv_kernel, use_radial_filters = channel_base * 2, channel_max * 2, 1, True

        # Dual-branch specific configuration
        # Get STYLEGAN3_DUAL config from the main config loader
        from config import Configurations
        temp_cfg = Configurations(None)
        STYLEGAN3_DUAL = temp_cfg.STYLEGAN3_DUAL if hasattr(temp_cfg, 'STYLEGAN3_DUAL') else None

        ct_channels = DATA.img_channels
        mask_channels = 1  # Binary mask

        # Get dual-branch specific parameters
        if STYLEGAN3_DUAL:
            cross_connection_layers = STYLEGAN3_DUAL.cross_connection_layers
            use_attention = STYLEGAN3_DUAL.use_attention
        else:
            # Default values if config not found
            cross_connection_layers = [4, 6, 8]
            use_attention = False
        ...
        Gen = module.Generator(
            ...
            synthesis_kwargs={
                ...
                # Dual-branch specific parameters
                "cross_connection_layers": cross_connection_layers,
                "use_attention": use_attention
            }
        ).to(device)

        # Generator has mapping and synthesis (dual-branch)
        Gen_mapping, Gen_synthesis = Gen.mapping, Gen.synthesis

        # Discriminator (dual-input for CT and mask)
        ...
        # EMA for dual-branch generator
        if MODEL.apply_g_ema:
            if device == 0:
                logger.info("Prepare exponential moving average generator for dual-branch StyleGAN3.")
            Gen_ema = copy.deepcopy(Gen)
            Gen_ema_mapping, Gen_ema_synthesis = Gen_ema.mapping, Gen_ema.synthesis
