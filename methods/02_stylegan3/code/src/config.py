...
class Configurations(object):
    ...
    def load_base_cfgs(self):
        ...
        # translation mode for image-to-image translation \in ["N/A", "unpaired", "paired"]
        self.DATA.translation_mode = "N/A"
        # domain A path for unpaired translation (relative to data_dir)
        self.DATA.domain_A_path = "trainA"
        # domain B path for unpaired translation (relative to data_dir)
        self.DATA.domain_B_path = "trainB"
        ...
        # -----------------------------------------------------------------------------
        # CycleGAN settings (for unpaired image-to-image translation)
        # -----------------------------------------------------------------------------
        self.CYCLEGAN = misc.make_empty_object()

        # Weight for cycle consistency loss (A -> B -> A)
        self.CYCLEGAN.lambda_A = 10.0
        # Weight for cycle consistency loss (B -> A -> B)
        self.CYCLEGAN.lambda_B = 10.0
        # Weight for identity loss (optional, set to 0 to disable)
        self.CYCLEGAN.lambda_identity = 0.5
        # Whether to use identity loss
        self.CYCLEGAN.use_identity_loss = True
        # Image buffer pool size for discriminator training (0 to disable)
        self.CYCLEGAN.pool_size = 50
        # Generator architecture type \in ["resnet_9blocks", "resnet_6blocks"]
        self.CYCLEGAN.gen_type = "resnet_9blocks"
        # Discriminator architecture type \in ["basic", "n_layers", "pixel"]
        self.CYCLEGAN.disc_type = "basic"
        # Normalization layer type \in ["instance", "batch", "none"]
        self.CYCLEGAN.norm = "instance"
        # Whether to use dropout in generator
        self.CYCLEGAN.use_dropout = False

        # -----------------------------------------------------------------------------
        # Pix2Pix settings (for paired image-to-image translation)
        # -----------------------------------------------------------------------------
        self.PIX2PIX = misc.make_empty_object()

        # Weight for L1 reconstruction loss
        self.PIX2PIX.lambda_L1 = 100.0
        # Generator architecture type \in ["unet_256", "unet_128", "resnet_9blocks", "resnet_6blocks"]
        self.PIX2PIX.gen_type = "unet_256"
        # Discriminator architecture type \in ["basic", "n_layers", "pixel"]
        self.PIX2PIX.disc_type = "basic"
        # Normalization layer type \in ["batch", "instance", "none"]
        self.PIX2PIX.norm = "batch"
        # Whether to use dropout in generator
        self.PIX2PIX.use_dropout = True
        # Number of input image channels
        self.PIX2PIX.input_nc = 3
        # Number of output image channels
        self.PIX2PIX.output_nc = 3

        # -----------------------------------------------------------------------------
        # StyleGAN3 Dual-Branch settings (for simultaneous CT and mask generation)
        # -----------------------------------------------------------------------------
        self.STYLEGAN3_DUAL = misc.make_empty_object()

        # Number of output channels for CT images
        self.STYLEGAN3_DUAL.ct_channels = 3
        # Number of output channels for mask images
        self.STYLEGAN3_DUAL.mask_channels = 1
        # Layer indices to apply cross-branch connections [list of int]
        # Typical values: [4, 6, 8] for connections at 64x64, 256x256, 1024x1024
        self.STYLEGAN3_DUAL.cross_connection_layers = [4, 6, 8]
        # Whether to use attention mechanism in cross-branch connections
        self.STYLEGAN3_DUAL.use_attention = False
        # Weight for dual-branch consistency loss (encourages coherent CT-mask generation)
        self.STYLEGAN3_DUAL.lambda_consistency = 1.0
        # Weight for CT discriminator loss
        self.STYLEGAN3_DUAL.lambda_ct = 1.0
        # Weight for mask discriminator loss
        self.STYLEGAN3_DUAL.lambda_mask = 1.0
        # Fusion strategy for discriminator \in ["average", "weighted", "separate"]
        # "average": simple average of CT and mask logits
        # "weighted": lambda_ct * ct_logits + lambda_mask * mask_logits
        # "separate": train CT and mask discriminators independently
        self.STYLEGAN3_DUAL.fusion_strategy = "weighted"
        # Channel reduction ratio for attention in FeatureFusionModule
        self.STYLEGAN3_DUAL.attention_reduction = 8
    ...
    def check_compatability(self):
        ...
        if self.MODEL.backbone == "deep_conv":
            # Patched: deep_conv now auto-scales via _make_dim_pyramid (see models/deep_conv.py).
            # Allow any power-of-two resolution >= 32.
            import math
            assert self.DATA.img_size >= 32 and (self.DATA.img_size & (self.DATA.img_size - 1)) == 0, \
                f"deep_conv requires img_size to be a power of 2 >= 32; got {self.DATA.img_size}"
