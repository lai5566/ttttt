#!/bin/bash
# MLA-GAN IR=20 訓練（原版 / 非 K-fold / batch=16）— 400ep, seed=7。
# ┌─[BD 標注]──────────────────────────────────────────────────────────┐
# │ 非 K-fold。on-the-fly 'rank' BD,由 resnet50_v3_ir20 引導分類器現算。│
# │ batch=16 版(矩陣 bs16 cell)。輸出到 output_ir20_v3_bs16/,不蓋 bs8。│
# │ 對照:train_ir20_v3.sh = bs8 原版;train_ir20.sh = K-fold。         │
# └────────────────────────────────────────────────────────────────────┘
set -euo pipefail
cd "$(dirname "$0")/code"
